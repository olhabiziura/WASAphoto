    ...
    methods: {
        async setMyUsername() {
            try {
				...
				let response = await this.$axios.put('/username', {username: username}, {headers: {'Authorization': `${sessionStorage.getItem('token')}`, 'Content-Type': 'application/json'}});
				//let user = response.data // user_id, username
				//sessionStorage.setItem('token', user.user_id);
				sessionStorage.setItem('username', username);
                alert(`Username correctly updated to: ${username}`)
